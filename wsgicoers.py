...
class CORS(object):
    ...
    def matchlist(origin, allowed_origins, case_sensitive=False):
        return reduce(
            lambda accu, x: CORS.matchpattern(
                accu, x, origin.lower() if not case_sensitive else origin
            ),
            allowed_origins,
            False,
        )
    ...
    def __init__(self, application, cfg=None, **kw):
        ...
        if kw and "policy" not in kw:  # direct config
        ...
        else:  # multiple policies programatically or
            # via configfile (paster factory for instance)
            cfg = kw or cfg or {}
            self.activepolicies = list(
                map(lambda x: x.strip(), cfg.get("policy", "deny").split(","))
            )
            ...
            for policy in self.activepolicies:
                ...
                prefix = policy + "_"
                for k, v in filter(
                    lambda key_value: key_value[0].startswith(prefix), cfg.items()
                ):
        ...
        for policy in self.activepolicies:
            ...
            # copy or * or a space separated list of hostnames,
            # possibly with filename wildcards "*" and "?"
            pol_origin = kw.get("origin", "")
            if pol_origin not in ("copy", "*"):
                match = list(
                    filter(
                        lambda x: x != "*",
                        map(lambda x: x.strip(), pol_origin.split(" ")),
                    )
                )
            else:
                match = []
            ...
            pol_expose_headers = kw.get(
                "expose_headers", ""
            )  # * or list of headers to expose to the client
            ...
            configkeys = (
                "origin,methods,headers,expose_headers,credentials,maxage".split(",")
            )
            ...
            if "origin" not in kw:
                if existingkeys:
                    print(
                        "The policy '%s' was referenced but has no value for "
                        "'origin' set. Nothing good can come from this." % policy
                    )
                elif policy != "deny":
                    print(
                        "The policy '%s' was referenced but hasn't defined any keys. "
                        "This might be an case sensitivity issue." % policy
                    )
    ...
    def selectPolicy(self, origin, request_method=None):
        """
        Based on the matching strategy and the origin and optionally
        the requested method a tuple of policyname
        and origin to pass back is returned.
        """
        ret_origin = None
        policyname = None
        if self.matchstrategy in ("firstmatch", "verbmatch"):
            for pol in self.activepolicies:
                ...
                if self.matchstrategy == "verbmatch":
                    if policy.methods != "*" and not CORS.matchlist(
                        request_method, policy.methods, case_sensitive=True
                    ):
                        continue
    ...
    def __call__(self, environ, start_response):
        ...
        if (
            "OPTIONS" == environ["REQUEST_METHOD"]
            and environ.get("HTTP_ACCESS_CONTROL_REQUEST_METHOD") is not None
            ...
            if policyname == "deny":
                ...
                if origin:
                    resp.append(("Access-Control-Allow-Origin", origin))
                if methods:
                    resp.append(("Access-Control-Allow-Methods", methods))
                if headers:
                    resp.append(("Access-Control-Allow-Headers", headers))
                if credentials:
                    resp.append(("Access-Control-Allow-Credentials", credentials))
                if maxage:
                    resp.append(("Access-Control-Max-Age", maxage))
